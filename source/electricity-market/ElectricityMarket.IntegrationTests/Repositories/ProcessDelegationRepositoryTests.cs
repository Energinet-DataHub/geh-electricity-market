...
public class ProcessDelegationRepositoryTests : IClassFixture<ElectricityMarketDatabaseContextFixture>, IAsyncLifetime
{
    private readonly ElectricityMarketDatabaseContextFixture _fixture;

    public ProcessDelegationRepositoryTests(ElectricityMarketDatabaseContextFixture fixture)
    {
        _fixture = fixture;
    }

    [Fact]
    public async Task GetProcessDelegationAsync_ReturnsSingleDelegation_WhenMultiplePeriodsExist()
    {
        // Arrange
        var context = _fixture.DatabaseManager.CreateDbContext();
        var sut = new ProcessDelegationRepository(context);

        var gridArea = new GridArea { Id = Guid.NewGuid(), Code = "804" };
        var actor = new Actor { Id = Guid.NewGuid(), ActorNumber = "123456", MarketRole = new MarketRole { Function = "BalanceResponsibleParty" } };
        var delegatedToActor = new Actor { Id = Guid.NewGuid(), ActorNumber = "654321", MarketRole = new MarketRole { Function = "BalanceSupplier" } };
        var processDelegation = new ProcessDelegation { Id = Guid.NewGuid(), DelegatedByActorId = actor.Id, DelegatedProcess = "SomeProcessType" };

        var delegationPeriods = new[]
        {
            new DelegationPeriod
            {
                Id = Guid.NewGuid(),
                ProcessDelegationId = processDelegation.Id,
                GridAreaId = gridArea.Id,
                DelegatedToActorId = delegatedToActor.Id,
                StartsAt = DateTimeOffset.UtcNow.AddDays(-10),
                StopsAt = DateTimeOffset.UtcNow.AddDays(10)
            },
            new DelegationPeriod
            {
                Id = Guid.NewGuid(),
                ...
                StopsAt = DateTimeOffset.UtcNow.AddDays(-5)
            }
        };

        context.GridAreas.Add(gridArea);
        context.Actors.Add(actor);
        context.Actors.Add(delegatedToActor);
        context.ProcessDelegations.Add(processDelegation);
        context.DelegationPeriods.AddRange(delegationPeriods);

        await context.SaveChangesAsync();

        var repository = new ProcessDelegationRepository(context);

        var request = new ProcessDelegationRequestDto
        {
            GridAreaCode = "804",
            ActorNumber = "123456",
        ...
        // Act
        var result = await repository.GetProcessDelegationAsync(request);

        // Assert
        Assert.NotNull(result);
        Assert.Equal("654321", result!.ActorNumber);
        Assert.Equal("BalanceSupplier", result.ActorRole);
    }
