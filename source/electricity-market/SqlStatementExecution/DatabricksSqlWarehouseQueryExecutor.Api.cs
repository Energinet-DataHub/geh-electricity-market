...
using Energinet.DataHub.Core.DatabricksExperimental.SqlStatementExecution.Formats;
...
public partial class DatabricksSqlWarehouseQueryExecutor
{
    ...
    public Task<(string StatementId, Chunks[] Chunks)> ExecuteChunkyStatementAsync(DatabricksStatement statement)
    {
        return ExecuteStatementGetChunksAsync(statement, QueryOptions.Default);
    }

    public IAsyncEnumerable<dynamic> ExecuteChunkyStatementAsync(string statementId, Chunks chunk)
    {
        return ExecuteStatementChunkAsync(QueryOptions.Default, statementId, chunk);
    }
