...
public partial class DatabricksSqlWarehouseQueryExecutor
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        await foreach (var p in ProcessChunksInParallel(cancellationToken, response, strategy, options.MaxParallelChunks).ConfigureAwait(false))
            yield return p;
    }
    ...
    private async Task<(string StatementId, Chunks[] Chunks)> ExecuteStatementGetChunksAsync(
        DatabricksStatement statement,
        QueryOptions options)
    {
        var strategy = options.Format.GetStrategy(_options);
        var request = strategy.GetStatementRequest(statement);
        var response = await request.WaitForSqlWarehouseResultAsync(_httpClient, StatementsEndpointPath, CancellationToken.None).ConfigureAwait(false);
        ...
        return (response.statement_id, response.manifest.chunks);
    }

    private async IAsyncEnumerable<dynamic> ExecuteStatementChunkAsync(
        QueryOptions options,
        string statementId,
        Chunks chunk)
    {
        ...
        var stream = await _externalHttpClient.GetStreamAsync(chunkResponse.external_links[0].external_link).ConfigureAwait(false);
        await using (stream.ConfigureAwait(false))
        {
            await foreach (var row in strategy.ExecuteAsync(stream, null!, CancellationToken.None).ConfigureAwait(false))
            {
                yield return row;
            }
        }
