...
public class CapacitySettlementService : ICapacitySettlementService
{
    ...
    {
        var closedDownTimestamp = capacitySettlementMeteringPoint.MetadataTimeline.OrderBy(period => period.Valid.Start)
            .FirstOrDefault(period => period.ConnectionState == ConnectionState.ClosedDown)?.Valid.Start;
        return closedDownTimestamp?.ToDateTimeOffset();
    }
    ...
    {
        if (dateTimeOffset < _capacitySettlementEnabledFrom.ToDateTimeOffset())
        {
            return _capacitySettlementEnabledFrom.ToDateTimeOffset();
        }
